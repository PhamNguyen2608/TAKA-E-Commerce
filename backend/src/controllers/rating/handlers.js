...
class RatingHandlers {
  ...
  async updateRating(_id, newData) {
    try {
      const query = { _id };
      const data = newData;
      const option = {
      ...
      const result = await RatingModel.findOneAndUpdate(query, data, option);

      if (!result || result === 'null')
        return {
          status: ERROR,
          response: new HttpException(400, 'Can not update rating!'),
        };
  ...
  async removeRating(_id) {
    try {
      ...
      if (!result || result === 'null')
        return {
          status: ERROR,
          response: new HttpException(400, 'Can not remove rating!'),
        };
      ...
      return {
        ...
        response: {
          status: OK,
          message: 'Remove rating successfully!',
          data: result,
        },
      };
    } catch (error) {
  ...
  async queryAllRates(params, conditions) {
    try {
      ...
      const results = await RatingModel.find(query)
  ...
  async getIsBought(data) {
    ...
    if (!userOrderItem || userOrderItem === 'null') {
      return {
        status: SUCCESS,
        response: {
          status: OK,
