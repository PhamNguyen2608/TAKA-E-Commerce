...
class NotificationControllers {
  ...
  async updateNotification(req, res, next) {
    try {
      ...
      // const data = DTO.updateNotification(req.body);
      const response = await NotificationHandlers.updateNotification(_id);
      console.log(
        '🚀 ~ file: index.js ~ line 28 ~ NotificationControllers ~ updateNotification ~ response',
        response
      );
